...
public class PBD_model: MonoBehaviour {
	...
	void Strain_Limiting()
	{
		...
		for (int i = 0; i < vertices.Length; i++) { sum_n[i] = 0; }

        for (int i = 0; i < E.Length / 2; i++)
        {
			int a0 = E[i * 2];
			int a1 = E[i * 2 + 1];
			Vector3 d = vertices[a0] - vertices[a1];
			Vector3 c = vertices[a0] + vertices[a1];

			sum_x[a0] += 0.5f * (c + L[i] * d.normalized);
			sum_x[a1] += 0.5f * (c - L[i] * d.normalized);
			sum_n[a0] += 1;
			sum_n[a1] += 1;
		}
